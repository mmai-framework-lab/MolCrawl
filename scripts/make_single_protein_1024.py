...
def unpack_split(src_path: Path, out_path: Path, split: str, limit: int) -> int:
    ...
    n = 0
    buf: list[list[int]] = []
    # Carried across blocks: the packed stream's block boundaries do not line up
    # with protein boundaries, so a protein routinely starts in one block and ends
    # in the next. Resetting this per block would emit the tail half as if it were a
    # whole protein and drop the head half - at ~3.1 proteins per block that is
    # roughly a third of the output missing its N-terminus.
    seg: list[int] = []
    truncated = 0
    done = False
    ...
    if truncated:
        logger.info(
            "  %s: %d of %d proteins were longer than %d and got cut to it (%.2f%%)",
            split, truncated, n, TARGET_LEN, 100 * truncated / n,
        )
