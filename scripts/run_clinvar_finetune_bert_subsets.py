"""Driver: fine-tune ClinVar classifier on every BERT subset checkpoint.

For each (subset, seed) tuple, invokes
``scripts/finetune_clinvar_bert.py`` and aggregates per-run AUROC into
a summary CSV / report. Mirrors the layout of
``run_clinvar_eval_subsets.py`` so downstream tooling can read both.

Usage:
    export LEARNING_SOURCE_DIR=/path/to/learning_source
    python scripts/run_clinvar_finetune_bert_subsets.py [--dry-run]
        [--subsets a,b,c] [--seeds 1,2,3] [--num-train-steps 500]

Output layout:
    $LEARNING_SOURCE_DIR/genome_sequence/analysis/clinvar_finetune_bert/
        bert__<subset>__seed<N>/
            metrics.json
            predictions.jsonl
            splits_manifest.csv
            trainer/  (huggingface trainer dir)
        clinvar_finetune_results.csv
        clinvar_finetune_summary.csv
        clinvar_finetune_report.txt
"""
...
def run_one(
    ...
    out_dir = out_root / f"bert__{subset}__seed{seed}"
    cmd = [
        python_exe, "scripts/finetune_clinvar_bert.py",
        "--model-path",     str(ckpt),
    ...
    # Cache hit (re-invocation): reuse if metrics.json exists.
    ...
    t0 = time.time()
    print(f"RUN: bert {subset} seed={seed} ...", flush=True)
    ...
    with out_path.open("w", newline="") as f:
        ...
        for s, vals in sorted(g.items()):
            if not vals:
                continue
            w.writerow({
                "subset": s, "model_type": "bert", "n_seeds": len(vals),
...
def main() -> int:
    ...
    out_dir_name = (
        f"clinvar_finetune_bert_{args.out_tag}" if args.out_tag
        else "clinvar_finetune_bert"
    )
    ...
    subsets = (
        ...
        else [s for s in ALL_SUBSETS if ckpt_path(base_dir, s).exists()]
    )
    ...
    for s in subsets:
        ...
        for seed in seeds:
            ...
            results.append(r)
            if not args.dry_run:
                # Incremental dump so partial progress isn't lost.
