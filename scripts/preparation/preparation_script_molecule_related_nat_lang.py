...
def run_statistics(series, column_name):
    ...
    plt.savefig(
        "assets/img/molecule_nl_tokenized_{}_lengths_dist.png".format(column_name)
    )
    plt.close()
    logger.info(
...
def analyze_dataset_tasks(dataset):
    ...
    for split in dataset.keys():
        ...
        if "sample_id" in dataset[split].features:
            ...
            logger.warning(
                f"No sample_id found in {split} split - cannot analyze task distribution"
            )
...
def calculate_statistics(dataset, split):
    inp_out = [
        i + j for i, j in zip(dataset[split]["input_ids"], dataset[split]["output_ids"])
    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--download",
        action="store_true",
        help="Download dataset from Hugging Face (default: use local dataset)",
    )
    parser.add_argument(
        "--local-path",
        type=str,
        default="molecule_related_nl/osunlp/SMolInstruct",
    ...
    base_dataset_dir = (
        Path(learning_source_dir) / "molecule_nl" / "osunlp" / "SMolInstruct"
    )
    logging_dir = Path(learning_source_dir) / "molecule_nl" / "logs"
    parquet_file = (
        Path(learning_source_dir)
        / "molecule_nl"
    ...
    if use_local_data:
        ...
        if (
            not args.force
            and dataset_marker_file.exists()
            and dataset_config_file.exists()
        ):
            logger.info(
                msg=f"Local dataset already copied to {base_dataset_dir}. Skipping copy."
            )
            logger.info(
                msg="If you want to re-copy, please use --force option or delete the directory and run again."
            )
        else:
            if args.force:
        ...
        if (
            ...
            logger.info(
                msg=f"Dataset already exists at {base_dataset_dir}. Skipping download."
            )
            logger.info(
                msg="If you want to re-download, please use --force option or delete the directory and run again."
            )
            ...
            try:
                ...
                logger.info(
                    msg=f"Dataset validation successful. Found splits: {list(test_dataset.keys())}"
                )
            except Exception as e:
            ...
            try:
                ...
                logger.error(
                    msg="This may be due to corrupted cache files. Trying to clear cache and retry..."
                )
                ...
                try:
                    download_hf_dataset(base_dataset_dir)
                    logger.info(
                        msg="Dataset download completed successfully after cache clear."
                    )
                except Exception as retry_error:
                    logger.error(
                        msg=f"Failed to download dataset even after cache clear: {retry_error}"
                    )
                    exit(1)
            except Exception as e:
                logger.error(msg="Failed to download dataset.")
                ...
                try:
                    ...
                    logger.info(
                        msg="Dataset download completed via alternative approach."
                    )

                except Exception as alt_error:
                    logger.error(
                        msg=f"Alternative download approach also failed: {alt_error}"
                    )
                    exit(1)
    ...
    try:
        ...
        logger.error(
            msg="This suggests corrupted dataset files. Removing and re-downloading..."
        )
    ...
    for split in dataset.keys():
        ...
        def validate_and_tokenize(example):
            ...
            def default_result():
                return {
                    ...
                    "task_type": example.get("sample_id", "unknown").split(".")[0]
            ...
            try:
                ...
                if not validate_smiles_in_sample(example):
                    logger.debug(
                        f"Skipping sample due to invalid SMILES: {example.get('sample_id', 'unknown')}"
                    )
                ...
                if not isinstance(result, dict):
                    logger.warning(
                        f"Tokenizer returned non-dict for sample {example.get('sample_id', 'unknown')}"
                    )
                    return default_result()

                result.setdefault(
                    "task_type",
                    example.get("sample_id", "unknown").split(".")[0]
                    if "sample_id" in example
                    else "unknown",
                )
                ...
                logger.warning(
                    f"Error processing sample {example.get('sample_id', 'unknown')}: {e}"
                )
                return default_result()
        ...
        processed_split = dataset[split].map(
            ...
            remove_columns=dataset[
                split
            ].column_names,  # Remove original columns to save space
        )
        ...
        logger.info(
            f"Processed {len(processed_dataset[split])} valid samples in {split} split"
        )
    ...
    for split in processed_dataset.keys():
        ...
        if "task_type" in processed_dataset[split].features:
            ...
            for task_type in set(task_types):
                ...
                task_tokens = sum(
                    len(processed_dataset[split][i]["input_ids"])
                    + len(processed_dataset[split][i]["output_ids"])
    ...
    if task_stats:
        ...
        for task_type, stats in sorted(task_stats.items()):
            logger.info(
                msg=f"{task_type}: {stats['samples']} samples, {stats['tokens']} tokens"
            )
