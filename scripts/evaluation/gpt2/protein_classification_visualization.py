...
class ProteinClassificationVisualizer(BaseVisualizationGenerator):
    ...
    def plot_fitness_score_distribution(self):
        ...
        # ヒストグラム
        plt.hist(benign_scores, bins=30, alpha=0.5, label='Benign', color='blue', edgecolor='black')
        plt.hist(pathogenic_scores, bins=30, alpha=0.5, label='Pathogenic', color='red', edgecolor='black')
        
        # 閾値線
        plt.axvline(x=self.threshold, color='green', linestyle='--', linewidth=2,
        ...
        plt.figtext(0.15, 0.95, stats_text, fontsize=9, 
    ...
    def create_summary_dashboard(self):
        ...
        ax4.plot(fpr, tpr, 'b-', linewidth=2, label=f'ROC (AUC={self.metrics["ROC-AUC"]:.3f})')
        ...
        ax5.plot(recall, precision, 'g-', linewidth=2, label=f'PR (AUC={self.metrics["PR-AUC"]:.3f})')
        baseline = np.mean(self.true_labels)
        ax5.plot([0, 1], [baseline, baseline], 'r--', alpha=0.5, label=f'Baseline={baseline:.3f}')
