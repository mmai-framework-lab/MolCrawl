...
class MoleculeNLValidatorVisualization(BaseVisualizationGenerator):
    ...
    def _plot_performance_metrics(self, ax):
        ...
        metric_values = [
            pm['accuracy'], pm['precision'], pm['recall'], pm['f1_score'],
            pm['roc_auc'], pm['pr_auc'], pm['sensitivity'], pm['specificity']
        ]
        
        colors = ['#3498db' if v >= 0.7 else '#e74c3c' if v < 0.5 else '#f39c12' 
    ...
    def _create_detailed_confusion_matrix(self):
        ...
        sns.heatmap(cm_array, annot=True, fmt='d', ax=ax, cmap='Blues',
    ...
    def _create_confidence_histogram(self):
        ...
        axes[0].hist(correct['prediction_score'], bins=30, alpha=0.7, 
                    label='Correct Predictions', color='green', edgecolor='black')
        axes[0].hist(incorrect['prediction_score'], bins=30, alpha=0.7, 
        ...
        axes[1].hist(pos_samples['prediction_score'], bins=30, alpha=0.7,
                    label='Positive Samples', color='#e74c3c', edgecolor='black')
        axes[1].hist(neg_samples['prediction_score'], bins=30, alpha=0.7,
                    label='Negative Samples', color='#3498db', edgecolor='black')
    ...
    def _create_error_analysis_plot(self):
        ...
        if len(fp) > 0:
            axes[0].hist(fp['prediction_score'], bins=20, color='orange', 
        ...
        if len(fn) > 0:
            axes[1].hist(fn['prediction_score'], bins=20, color='purple',
                        alpha=0.7, edgecolor='black')
    ...
    def _generate_html_report(self):
        ...
        if self.metrics:
            ...
            pm = {'accuracy': 0, 'precision': 0, 'recall': 0, 'f1_score': 0,
                  'roc_auc': 0, 'pr_auc': 0, 'sensitivity': 0, 'specificity': 0}
            cm = {'true_positive': 0, 'false_positive': 0, 'true_negative': 0, 'false_negative': 0}
            di = {'total_samples': len(self.df), 'positive_samples': 0, 'negative_samples': 0}
