"""Tier B verification — per-token & per-position cross-entropy loss diff (genome).

Adapted from scripts/eval_protein_tier_b.py for genome_sequence × gpt2-small.
The protein version targets the ESM tokenizer (33 vocab, biology-rich
amino-acid alphabet); this one targets the SentencePiece BPE tokenizer
(~4096 vocab, k-mer-shaped pieces like 'TT', 'AGAC', 'GATAA') used by the
genome pipeline.

Per-token aggregation here is necessarily noisier because the BPE pieces
have skewed counts — short pieces like 'A'/'T'/'G'/'C' dominate.  The
script reports the top/bottom-K pieces by old-vs-new Δ rather than a
full table.
"""
...
def load_genome_vocab(spm_path: str) -> dict[int, str]:
    import sentencepiece as spm

    tok = spm.SentencePieceProcessor(model_file=spm_path)
    return {i: tok.id_to_piece(i) for i in range(tok.vocab_size())}
...
def main():
    ...
    ap.add_argument("--top_k", type=int, default=20, help="report top/bottom K pieces by Δ")
    ap.add_argument("--out_json", default="tier_b_genome_results.json")
    ...
    print(f"Loading SentencePiece vocab from {args.spm_model}...", flush=True)
    vocab = load_genome_vocab(args.spm_model)
    ...
    for tid in token_ids:
        ...
        piece = vocab.get(tid, f"<{tid}>")
        rows.append((tid, piece, max(o["n"], nn["n"]), o["avg"], nn["avg"], delta, ratio))

    # Top-K improvements (old > new, largest Δ)
    print(f"\n########## top-{args.top_k} BIGGEST IMPROVEMENT (Δ = old - new) ##########")
    print(f"{'tok_id':>6}  {'piece':>12}  {'count':>8}  {'old_loss':>10}  {'new_loss':>10}  {'Δ':>10}  {'×':>6}")
    for tid, piece, n, ol, nl, d, r in sorted(rows, key=lambda r: -r[5])[: args.top_k]:
        print(f"{tid:>6}  {piece:>12}  {n:>8}  {ol:>10.4f}  {nl:>10.4f}  {d:>10.4f}  {r:>6.2f}")

    # Top-K regressions (old < new)
    print(f"\n########## top-{args.top_k} BIGGEST REGRESSION (Δ negative = new is worse) ##########")
    print(f"{'tok_id':>6}  {'piece':>12}  {'count':>8}  {'old_loss':>10}  {'new_loss':>10}  {'Δ':>10}  {'×':>6}")
    for tid, piece, n, ol, nl, d, r in sorted(rows, key=lambda r: r[5])[: args.top_k]:
        print(f"{tid:>6}  {piece:>12}  {n:>8}  {ol:>10.4f}  {nl:>10.4f}  {d:>10.4f}  {r:>6.2f}")

    # Most-frequent K pieces
