...
if __name__ == "__main__":
    ...
    if learning_source_dir is None:
        logger.error("LEARNING_SOURCE_DIR environment variable is not set")
        exit(1)
    ...
    for split in dataset.keys():
        ...
        def validate_and_tokenize(example):
            try:
                ...
                result['task_type'] = example.get('sample_id', 'unknown').split('.')[0] if 'sample_id' in example else 'unknown'
                return result
            except Exception as e:
