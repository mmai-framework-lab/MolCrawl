#!/bin/bash
# Fine-tune the compounds GPT-2 (medium) model on GuacaMol benchmark data.
#
# Prerequisites:
...
#   bash workflows/03a-compounds_guacamol-train-medium.sh
...
run_training_background "${LOG_DIR}/compounds_guacamol-train-medium-$(date +%Y-%m-%d_%H-%M-%S).log" \
    molcrawl/models/gpt2/train.py \
    molcrawl/tasks/pretrain/configs/compounds/gpt2_guacamol_medium.py
