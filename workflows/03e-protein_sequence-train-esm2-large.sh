#!/bin/bash
#
# ESM-2 Training Script for Protein Sequence Data (Large Model)
#
...
# Log file path
LOG_FILE="${LEARNING_SOURCE_DIR}/protein_sequence/logs/esm2-train-large-$(date +%Y-%m-%d_%H-%M-%S).log"

echo "========================================"
echo "🧬 ESM-2 Training - Large Model"
echo "========================================"
...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES} nohup bash -c \
    '$PYTHON molcrawl/esm2/main.py esm2/configs/protein_sequence.py --model_size=large' \
    > "${LOG_FILE}" 2>&1 &
