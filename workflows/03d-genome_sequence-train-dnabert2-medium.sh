#!/bin/bash
#
# DNABERT-2 Training Script for Genome Sequence Data (Medium Model)
#
...
# Log file path
LOG_FILE="${LEARNING_SOURCE_DIR}/genome_sequence/logs/dnabert2-train-medium-$(date +%Y-%m-%d_%H-%M-%S).log"

echo "========================================"
echo "🧬 DNABERT-2 Training - Medium Model"
echo "========================================"
...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES} nohup bash -c \
    '$PYTHON molcrawl/dnabert2/main.py molcrawl/dnabert2/configs/genome_sequence.py --model_size=medium' \
    > "${LOG_FILE}" 2>&1 &
