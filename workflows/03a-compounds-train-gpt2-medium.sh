#!/bin/bash
# Pre-train the compounds GPT-2 (medium) model on organix13 SMILES.
#
# Prerequisites:
...
#   bash workflows/03a-compounds-train-gpt2-medium.sh
...
run_training_background "${LOG_DIR}/compounds-train-gpt2-medium-$(date +%Y-%m-%d_%H-%M-%S).log" \
    molcrawl/models/gpt2/train.py \
    molcrawl/tasks/pretrain/configs/compounds/gpt2_medium.py
