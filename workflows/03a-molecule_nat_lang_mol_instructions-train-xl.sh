#!/bin/bash
# Fine-tune the molecule_nat_lang GPT-2 (xl) model on Mol-Instructions.
#
# Prerequisites:
#   - molecule_nat_lang GPT-2 xl pretraining checkpoint must exist in
#       $LEARNING_SOURCE_DIR/molecule_nat_lang/gpt2-output/molecule_nat_lang-xl/
...
#   bash workflows/03a-molecule_nat_lang_mol_instructions-train-xl.sh
...
run_training_background "${LOG_DIR}/molecule_nat_lang_mol_instructions-train-xl-$(date +%Y-%m-%d_%H-%M-%S).log" \
    molcrawl/models/gpt2/train.py \
    molcrawl/tasks/pretrain/configs/molecule_nat_lang/gpt2_mol_instructions_xl.py

echo "GPT-2 xl Mol-Instructions fine-tuning running in background (GPU ${CUDA_VISIBLE_DEVICES})."
echo "Logs: ${LOG_DIR}/"
