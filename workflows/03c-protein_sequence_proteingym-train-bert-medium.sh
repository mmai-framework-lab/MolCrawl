#!/bin/bash
# Fine-tune the protein_sequence BERT (medium) model on ProteinGym DMS data.
#
# Prerequisites:
#   - protein_sequence BERT pretraining checkpoint must exist in
#       $LEARNING_SOURCE_DIR/protein_sequence/bert-output/protein_sequence-medium/
...
#   bash workflows/03c-protein_sequence_proteingym-train-bert-medium.sh
...
run_training_background "${LOG_DIR}/protein_sequence_proteingym-train-bert-medium-$(date +%Y-%m-%d_%H-%M-%S).log" \
    molcrawl/models/bert/main.py \
    molcrawl/tasks/pretrain/configs/protein_sequence/bert_proteingym_medium.py
