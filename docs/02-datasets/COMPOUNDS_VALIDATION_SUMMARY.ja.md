...
`compounds-validation.yml` の `integration-tests` ジョブは、現在以下を実行します：

```bash
pytest tests/unit/test_compounds.py -m "integration and compound" -v
```

#### 参考: 詳細な統合テストファイル（ローカル実行）

`tests/integration/test_compounds_pipeline.py` には以下のテストがあります：
...
### 3. テスト用フィクスチャ（`tests/conftest.py`）
...
| ファイル                             | 内容                           | 対象読者 |
| ------------------------------------ | ------------------------------ | -------- |
| **COMPOUNDS_VALIDATION_GUIDE.ja.md**    | 包括的な検証ガイド             | 全員     |
| **COMPOUNDS_VALIDATION_EXAMPLES.ja.md** | 実践的な使用例                 | 開発者   |
| **COMPOUNDS_VALIDATION_SUMMARY.ja.md**  | セットアップサマリー（本文書） | 全員     |
...
3.  モデルパスを設定してBERT/GPT-2統合テストを実行
...
- **詳細ガイド**: [COMPOUNDS_VALIDATION_GUIDE.ja.md](COMPOUNDS_VALIDATION_GUIDE.ja.md)
- **実践例**: [COMPOUNDS_VALIDATION_EXAMPLES.ja.md](COMPOUNDS_VALIDATION_EXAMPLES.ja.md)
