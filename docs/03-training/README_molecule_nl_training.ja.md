...
このステップでは `molcrawl/preparation/preparation_script_molecule_related_nat_lang.py` を実行し、以下を作成します。

- `${LEARNING_SOURCE_DIR}/molecule_nl/molecule_related_natural_language_tokenized.parquet`
- `${LEARNING_SOURCE_DIR}/molecule_nl/arrow_splits/`（分割済みデータセット）
- `${LEARNING_SOURCE_DIR}/molecule_nl/gpt2_format/`（トークンストリーム `.pt` ファイル）
...
- 互換性テスト: `molcrawl/preparation/test_molecule_nl_compatibility.py`
- データセット比較レポート: `docs/07-reports/molecule_nl_dataset_comparison_report.md`
