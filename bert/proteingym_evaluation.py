...
class BERTProteinGymEvaluator:
    ...
    def save_results(self, results, output_dir):
        ...
        if 'detailed_results' in results:
            ...
            detailed_df.to_csv(os.path.join(output_dir, 'bert_proteingym_detailed_results.csv'), index=False)
    ...
    def _create_report(self, results, output_dir):
        ...
        with open(report_path, 'w') as f:
            ...
            f.write(f"   • True Score Range: {results['true_score_stats']['min']:.4f} to {results['true_score_stats']['max']:.4f}\n")
            f.write(f"   • Predicted Score Range: {results['predicted_score_stats']['min']:.4f} to {results['predicted_score_stats']['max']:.4f}\n")
            f.write(f"   • True Score Mean: {results['true_score_stats']['mean']:.4f} ± {results['true_score_stats']['std']:.4f}\n")
            f.write(f"   • Predicted Score Mean: {results['predicted_score_stats']['mean']:.4f} ± {results['predicted_score_stats']['std']:.4f}\n\n")
