...
class BERTVisualizationGenerator:
    ...
    def create_correlation_plot(self) -> str:
        ...
        z = np.polyfit(x, y, 1)
        p = np.poly1d(z)
        ax1.plot(x, p(x), "r--", alpha=0.8, linewidth=2, label=f'Fit: y = {z[0]:.3f}x + {z[1]:.3f}')
        ...
        # Residual plot
        residuals = y - x
        ax2.scatter(x, residuals, alpha=0.6, s=50, color='orange', edgecolors='white', linewidth=0.5)
    ...
    def create_distribution_plots(self) -> str:
        ...
        ax1 = axes[0, 0]
        ax1.hist(self.detailed_results['true_score'], bins=30, alpha=0.7, color='steelblue', 
                edgecolor='black', label='Actual DMS')
        ax1.hist(self.detailed_results['predicted_fitness'], bins=30, alpha=0.7, color='orange', 
                edgecolor='black', label='BERT Predicted')
        ...
        if len(actual_pos) > 0 and len(bert_pos) > 0:
            ax2.hist(np.log10(actual_pos), bins=20, alpha=0.7, color='steelblue', 
                    edgecolor='black', label='log10(Actual DMS)')
            ax2.hist(np.log10(bert_pos), bins=20, alpha=0.7, color='orange', 
                    edgecolor='black', label='log10(BERT Predicted)')
            ax2.set_xlabel('log10(Score)', fontsize=12)
            ax2.set_ylabel('Frequency', fontsize=12)
            ax2.set_title('Log-Scale Distribution', fontsize=14, fontweight='bold')
            ax2.legend()
        else:
            ax2.text(0.5, 0.5, 'No positive scores\nfor log transformation', 
        ...
        ax3.set_title('Q-Q Plot: Actual DMS Scores vs Normal Distribution', fontsize=12, fontweight='bold')
        ...
        box_plot = ax4.boxplot(data_to_plot, labels=['Actual DMS', 'BERT Predicted'], 
    ...
    def create_performance_metrics_plot(self) -> str:
        ...
        p_values = [
            self.results['spearman_p_value'],
            self.results['pearson_p_value']
        ]
        
        bars1 = ax1.bar(['Spearman ρ', 'Pearson r'], correlations, 
        ...
        for bar, p_val in zip(bars1, p_values):
            ...
            ax1.text(bar.get_x() + bar.get_width()/2., height + 0.05 * np.sign(height),
                    f'{significance}\np={p_val:.3f}', ha='center', va='bottom' if height > 0 else 'top',
                    fontsize=10)
        ...
        bars2 = ax2.bar(error_names, error_metrics, color=['green', 'red'], alpha=0.7, edgecolor='black')
    ...
    def create_summary_report(self) -> str:
        ...
        if self.detailed_results is not None:
            ...
            low_corr = stats.spearmanr(low_scores['true_score'], low_scores['predicted_fitness'])[0] if len(low_scores) > 1 else 0
            high_corr = stats.spearmanr(high_scores['true_score'], high_scores['predicted_fitness'])[0] if len(high_scores) > 1 else 0
