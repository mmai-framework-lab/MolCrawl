"""
Helper script for Llama-style checkpoint testing.

Note: the underlying ``test_checkpoint.py`` script currently raises
NotImplementedError because it targets the HF GPT-2 format, which is not valid
for the Llama-style architecture. This helper is preserved for path discovery
but the generated command will fail until a Llama-aware checkpoint tester is
implemented.

# List checkpoints
python -m molcrawl.models.llama.test_helper --search_dir=runs_train_llama_*/checkpoints --list_only

# test a specific checkpoint
python -m molcrawl.models.llama.test_helper --checkpoint_path=path/to/ckpt.pt

# Automatically test all checkpoints
python -m molcrawl.models.llama.test_helper --search_dir=runs_* --auto_run
...
def create_test_command(checkpoint_path, domain=None, output_dir=None, max_samples=500):
    ...
    cmd = [
        "python",
        "-m",
        "molcrawl.models.llama.test_checkpoint",
...
def main():
    parser = argparse.ArgumentParser(description="Llama-style checkpoint test helper")
...
def create_test_configs():
    ...
    configs_dir = Path("llama/test_configs")
    configs_dir.mkdir(exist_ok=True)

    domain_info = get_domain_info()

    for domain, info in domain_info.items():
        config_content = f"""# {domain.upper()} Llama-style test configuration for domain
