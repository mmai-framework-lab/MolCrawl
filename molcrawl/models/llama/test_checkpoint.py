...
# Inherited from the GPT-2 test harness; the HF-export portions are not valid
# for the Llama-style architecture (RMSNorm + RoPE + SwiGLU) and main() refuses
# to run. The perplexity / generation helpers below can be reused if adapted.
...
def main():
    raise NotImplementedError(
        "molcrawl.models.llama.test_checkpoint is inherited from the GPT-2 test "
        "harness and converts checkpoints to HuggingFace GPT2LMHeadModel format, "
        "which is not valid for the Llama-style architecture (RMSNorm + RoPE + "
        "SwiGLU). Use the perplexity / generation helpers in this module directly "
        "if needed, or implement a LlamaForCausalLM-targeted equivalent."
    )
