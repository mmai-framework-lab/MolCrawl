...
# -----------------------------------------------------------------------------
# default config values for a Llama-style decoder (RMSNorm + RoPE + SwiGLU) at GPT-2-small size
# I/O
...
wandb_project = os.environ.get("WANDB_PROJECT", "llama-training")  # wandb project name
...
# 'scratch' or 'resume'. 'gpt2*' is rejected: HF GPT-2 weights are not compatible with
# the Llama-style architecture (RMSNorm + RoPE + SwiGLU).
init_from = "scratch"
pretrain_dir = ""  # Path to pretraining out_dir; used when init_from='resume' and out_dir has no checkpoint
# checkpoint management
# save_hf_checkpoints uses a GPT-2-format converter that is NOT valid for the
# Llama-style arch, so it defaults to False here (in contrast to the legacy gpt2/
# train.py). Use legacy ckpt.pt instead.
...
keep_legacy_ckpt = True  # save ckpt.pt; required when save_hf_checkpoints is False
# early stopping
...
if __name__ == "__main__":
    ...
    elif os.path.exists("src/llama/configurator.py"):
        configurator_path = "src/llama/configurator.py"
    elif os.path.exists("llama/configurator.py"):
        configurator_path = "llama/configurator.py"
    else:
    ...
    # Resolve ambiguous token ids once for CLM-side loss masking.
    # `ambiguous_token_ids` is read by get_batch() at module level.
...
if __name__ == "__main__":
    ...
    if init_from == "scratch":
        ...
        # The Llama-style arch (RMSNorm + RoPE + SwiGLU) is not weight-compatible
        # with HF GPT-2 weights. Use init_from='scratch' or 'resume' instead.
        raise NotImplementedError(
            f"init_from={init_from!r} is not supported in the Llama-style train loop. "
            "Use init_from='scratch' or init_from='resume'."
        )
...
def convert_nanogpt_to_hf_state_dict(model_state, model_args):
    """Deprecated: not valid for Llama-style architecture.

    The conversion target (HuggingFace ``GPT2LMHeadModel``) uses LayerNorm +
    learnable positional embeddings + GELU MLP — none of which are present in
    the Llama-style decoder (RMSNorm + RoPE + SwiGLU). A correct exporter would
    need to target ``LlamaForCausalLM`` instead; that is out of scope for the
    current phase. Use the legacy ``ckpt.pt`` format (set ``keep_legacy_ckpt=True``
    and ``save_hf_checkpoints=False``) until such an exporter is implemented.
    """
    raise NotImplementedError(
        "HF GPT-2 export is not valid for the Llama-style architecture. "
        "Set save_hf_checkpoints=False and keep_legacy_ckpt=True, or implement "
        "a LlamaForCausalLM exporter."
    )
...
def save_checkpoint_hf(
    ...
    """Deprecated: produces a malformed HF GPT-2 export for the Llama-style arch.

    See ``convert_nanogpt_to_hf_state_dict`` for the explanation. The Llama-style
    train loop defaults to ``save_hf_checkpoints=False`` so this function is not
    called in normal use; if reached, it fails fast.
    """
    raise NotImplementedError(
        "save_checkpoint_hf targets HF GPT-2 format, which is not valid for the "
        "Llama-style architecture. Set save_hf_checkpoints=False (default for the "
        "Llama train loop) and rely on keep_legacy_ckpt=True to write ckpt.pt."
    )
