...
if __name__ == "__main__":
    ...
    wandb_project = os.environ.get("WANDB_PROJECT", "roberta-training")  # wandb project name
    ...
    elif os.path.exists("src/roberta/configurator.py"):
        configurator_path = "src/roberta/configurator.py"
    elif os.path.exists("roberta/configurator.py"):
        configurator_path = "roberta/configurator.py"
    else:
    ...
    # NB: HuggingFace RobertaEmbeddings uses position_ids starting from padding_idx+1
    # (typically 2), so the position embedding table must have size max_length + 2,
    # not max_length. This is an implementation detail of HF RoBERTa, unrelated to
    # the actual sequence length of input tokens.
    if model_size == "small":
        model_config = RobertaConfig(vocab_size=meta_vocab_size, max_position_embeddings=max_length + 2)
    elif model_size == "medium":
        # Sized to match gpt2-medium; named 'medium' for parity with the legacy bert/ naming.
    ...
    if _has_finetune_ckpt:
        ...
        model = RobertaForMaskedLM.from_pretrained(_load_path, config=model_config, ignore_mismatched_sizes=True)
    else:
    ...
    if use_wandb:
        ...
        if wandb_run_name is None:
            ...
            wandb_run_name = f"roberta-{model_size}-{timestamp}"

        # Determine dataset name from config
        dataset_name = config.get("dataset_name", "unknown")

        # Add metadata tags for experiment management
        tags = ["roberta", "training", model_size, dataset_name]
    ...
    training_args = TrainingArguments(
        ...
        # Hardware perf opt-ins (read from per-config globals; default off so
        # existing configs are unaffected). Ampere+/Blackwell configs flip these on.
        ...
        #   ddp_find_unused_parameters     : RoBERTa uses every parameter each step,
    ...
    try:
        ...
        if resume_checkpoint and (
            ...
            if pretrain_model_path and os.path.exists(pretrain_model_path):
                ...
                trainer.model = RobertaForMaskedLM.from_pretrained(_load_path, config=model_config, ignore_mismatched_sizes=True)
            else:
