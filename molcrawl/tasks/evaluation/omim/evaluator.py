"""OMIM disease-association evaluator."""
...
class OMIMEvaluator(BaseEvaluator):
    ...
    def __init__(
        ...
        self.omim_path = Path(omim_path)
        self.context_length: int = int(self.config.get("context_length", 512))
        self.category_column: str = str(self.config.get("category_column", "disease_category"))

    def category(self) -> str:
        return "variant_effect"

    def load_dataset(self) -> pd.DataFrame:
        df = load_omim(self.omim_path, category_column=self.category_column)
    ...
    def run_predictions(self, dataset: pd.DataFrame):
        ...
        return {"scores": ref_ll - var_ll, "reference_ll": ref_ll, "variant_ll": var_ll}

    def compute_metrics(self, dataset: pd.DataFrame, predictions) -> Dict[str, float]:
        scores = predictions["scores"]
        labels = dataset["omim_label"].to_numpy(dtype=int)
