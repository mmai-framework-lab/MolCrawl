...
class BaseVisualizationGenerator(ABC):
    ...
    def _create_comprehensive_dashboard(self, results_df: pd.DataFrame, 
        ...
        if similarity_col and similarity_col in results_df.columns:
            ...
            axes[0, 1].text(0.5, 0.5, 'Similarity data\nnot available', 
                           ha='center', va='center', transform=axes[0, 1].transAxes)
            axes[0, 1].set_title('Similarity Distribution (N/A)')
        
        # 散布図: スコア vs 類似度
        if similarity_col and similarity_col in results_df.columns:
            self._plot_score_vs_similarity_scatter(axes[0, 2], results_df, 
                                                 prediction_score_col, similarity_col, true_label_col)
        else:
            axes[0, 2].text(0.5, 0.5, 'Scatter plot\nnot available', 
    ...
    def _plot_similarity_distribution(self, ax, results_df: pd.DataFrame, sim_col: str, label_col: str):
        ...
        try:
            ...
            ax.text(0.5, 0.5, f'Similarity distribution\nerror: {str(e)}', 
                   ha='center', va='center', transform=ax.transAxes)
            ax.set_title('Similarity Distribution (Error)')
    
    def _plot_score_vs_similarity_scatter(self, ax, results_df: pd.DataFrame, 
                                        score_col: str, sim_col: str, label_col: str):
        """スコア vs 類似度の散布図"""
        try:
            positive_data = results_df[results_df[label_col] == 1]
            negative_data = results_df[results_df[label_col] == 0]
            
            ax.scatter(positive_data[score_col], positive_data[sim_col], 
                      alpha=0.6, c='red', label='Positive', s=20)
            ax.scatter(negative_data[score_col], negative_data[sim_col], 
                      alpha=0.6, c='blue', label='Negative', s=20)
            ax.set_xlabel('Prediction Score')
            ax.set_ylabel('Similarity Score')
            ax.set_title('Score vs Similarity')
            ax.legend()
        except Exception as e:
            ax.text(0.5, 0.5, f'Scatter plot\nerror: {str(e)}', 
                   ha='center', va='center', transform=ax.transAxes)
    ...
    def _plot_confidence_distribution(self, ax, results_df: pd.DataFrame, conf_col: str):
        ...
        try:
            ...
            ax.text(0.5, 0.5, f'Confidence distribution\nerror: {str(e)}', 
                   ha='center', va='center', transform=ax.transAxes)
