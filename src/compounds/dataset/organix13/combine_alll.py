...
if __name__ == "__main__":
    ...
    df_zinc_full = pd.read_parquet(os.path.join(cwd, "zinc", "zinc_processed.parquet"))
    df_zinc_full = df_zinc_full.sample(n=5_000_000)
    df_zinc_full = calculateProperties(df_zinc_full)

    print("df_zinc_qm9")
    df_zinc_qm9 = pd.read_parquet(
        os.path.join(cwd, "qm9_zinc250k_cep", "qm9_zinc250_cep.parquet")
    )
    ...
    print("df_chembl")
    df_chembl = pd.read_parquet(os.path.join(cwd, "chembl_log_sascore.parquet"))
    df_chembl = calculateProperties(df_chembl)

    print("df_pubchemqc_2017")
    df_pubchemqc_2017 = pd.read_parquet(os.path.join(cwd, "pubchemqc_energy.parquet"))
    ...
    df.to_parquet(os.path.join(cwd, "OrganiX13.parquet"))
