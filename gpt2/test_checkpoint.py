...
def main():
    ...
    parser.add_argument("--test_dataset_params", type=str, help="テストデータセットのパラメータ (JSON形式)")
    parser.add_argument("--domain", choices=["compounds", "molecule_nl", "genome", "protein_sequence", "rna"], 
                       help="使用するドメイン")
    parser.add_argument("--vocab_path", help="語彙ファイルのパス")
    parser.add_argument("--max_test_samples", type=int, default=1000, help="テストに使用する最大サンプル数")
    ...
    if test_dataset is None:
        ...
        # ダミーデータセットを作成（適切なサイズ）
        dummy_data = [torch.randint(0, model_args["vocab_size"], (model_args["block_size"],)) 
                     for _ in range(100)]
